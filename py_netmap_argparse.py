...
if __name__ == '__main__':	
	try:
		...
		parser.add_argument('-p', type=int, help="Ports seperated by space. EX: -p 21 22 80 443", nargs='+' ,required=False)
		parser.add_argument('-t', type=str, help='[Dotted Decimal] <target> EX: -t 192.168.1-5.1-254', nargs=1, required=True)
		parser.add_argument('--ttl', type=int, help='[int] <ttl> EX: --ttl 32 ', nargs=1, required=False)
		parser.add_argument('--timeout', type=float, help='[Float] [Seconds] <Timeout> EX: --timeout 0.5 (500 miliseconds)', nargs=1, required=False)
		...
		if cmdargs.ttl != None:
			...
			ttl=32
		if cmdargs.timeout != None:
			timeout=cmdargs.timeout[0]
		else:
			timeout=0.1	
		scan = Pynettop(host, time=timeout, ttl=ttl)
		ipup,ipdown,trace=scan.start
		ipup=scan.port_scanner(hosts=ipup, ports=ports, ttl=ttl, time=timeout)
		for ip in ipup:
			print("\n"+ip)
			print("port", "\t", "status")
